...
## Running FunMap:
python3 /home/sjozashoori/FunMap-master/engines/FunMap/run_translator.py config_rdb.ini
...
java -jar /home/sjozashoori/FunMap_ISWC2020/rmlmapper/rmlmapper.jar -m transfered_mapping_joinInQuery_rmlmapper.ttl -o output.nt

dur=$(echo "$(date +%s.%N) - $start" | bc)
echo "$data,$mapping,$dur"  >> results-rdb-FunMap-rmlmapper.csv
...
start=$(date +%s.%N)
java -jar rmlmapper.jar -m complex_sameFunction10_RDB_rmlmapper.ttl -f functions_complex.ttl -o output-rdb.nt -d
dur=$(echo "$(date +%s.%N) - $start" | bc)
echo "$data,$mapping,$dur"  >> results-rdb-rmlmapper.csv
