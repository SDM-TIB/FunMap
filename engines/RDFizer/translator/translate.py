	...
	for triples_map in triple_maps:
		...
		if triples_map.subject_map is not None:
			...
			if triples_map.subject_map.rdf_class is not None:
				mapping += "		rr:class " + triples_map.subject_map.rdf_class + "\n"
			mapping += "	];\n"
		...
		for predicate_object in triples_map.predicate_object_maps_list:
			...
			if "constant" in predicate_object.predicate_map.mapping_type :
				mapping += "		rr:predicate " + predicate_object.predicate_map.value + ";\n"
			...
			if "constant" in predicate_object.object_map.mapping_type:
				...
				mapping += "		]\n"
			elif "reference" in predicate_object.object_map.mapping_type:
				mapping += "[\n"
...
def update_csv(source, dic):
	with open(source, "r") as source_csv:
		with open("temp.csv", "w") as temp_csv:
			...
			print(keys)
			keys.append(dic["executes"])
			writer.writerow(keys)

			for row in reader:
				row[dic["executes"]] = "execute_function(row,dic)"
				line = []
				for key in row.keys():
					line.append(row[key])
			...
			for triples_map in triples_map_list:
				if str(triples_map.file_format).lower() == "csv" and triples_map.query == "None":
					#with open(str(triples_map.data_source), "r") as input_file_descriptor:
						...
						#executor.submit(semantify_file, triples_map, triples_map_list, ",", output_file_descriptor, wr, config[dataset_i]["name"], data).result()
					if triples_map.function:
						dic = create_dictionary(triples_map)
						update_csv(triples_map.data_source, dic)
						function_dic[triples_map.triples_map_id] = dic
					else:
						print(triples_map)	
				else:
