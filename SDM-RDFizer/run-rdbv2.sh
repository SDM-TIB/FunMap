...
############################# Running arrays ##############################
echo "---------Experiments over RDB with SDM-RDFizer (note that experiments with complex_sameFunction10_RDB_5millionRecords.ttl could not finish in SDM-RDFizer basic configuration---------"

declare -a mappings=("complex_sameFunction10_RDB.ttl" "complex_sameFunction10_RDB_5millionRecords.ttl")

## create result files
echo "mapping,time,iteration" > results-rdb-FunMap-SDM.csv
echo "mapping,time,iteration" > results-rdb-SDM.csv
...
do
	...
	for i in 1 2 3 4 5
	do
		echo "---------Running mapping $mapping iteration---------"
		start=$(date +%s.%N)
		## Running FunMap:
		...
		echo "$mapping,$dur,$i"  >> results-rdb-FunMap-SDM.csv
